#!/usr/bin/env bash
# Handicapping desk — Claude Code project scaffold
# Run:  bash bootstrap.sh   then:  cd ~/handicap && claude

set -e
ROOT="$HOME/handicap"
mkdir -p "$ROOT"/{data/raw,data/cache,scripts,reports,manual}
...
# ------------------------------------------------------------ slash commands
cat > .claude/commands/slate.md <<'EOF'
---
...
cat > .claude/commands/game.md <<'EOF'
---
description: Deep workup on one matchup
---
Full workup on the game I name: every available split source, line history with opener
and direction of travel, complete matchup and injury layer, verdict with the number and
the best book. If the splits are unverified, say so and analyze the matchup only.
EOF

cat > .claude/commands/paste.md <<'EOF'
---
description: Ingest pasted screenshots or text from gated sources
---
I am pasting content from a gated source (X post, Pikkit, MGM, FanDuel, Action PRO).
Save it verbatim to manual/YYYY-MM-DD-<source>.md, extract the figures into desk.db
tagged USER-PROVIDED with the account handle and the post timestamp, and tell me what it
changes about the current read. If it conflicts with a fetched number for the same book,
the pasted insider number wins and you flag the conflict.
EOF

cat > .claude/commands/grade.md <<'EOF'
---
description: Grade prior plays with CLV
---
Grade every logged play from the period I name. Report record, units won or lost, CLV on
each play, and a short honest note on what the losing plays had in common. No spin.
EOF
...
bankroll: 0
unit_size_pct: 1
max_units_per_play: 3
max_units_per_slate: 15
books_i_have: []   # e.g. [DraftKings TX-adjacent, BetOnline, Circa]
EOF
